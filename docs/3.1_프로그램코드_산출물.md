...
### 1.2 Frontend - 인증 컴포넌트 (components/auth/)

| 서브시스템명 | 프로그램파일 ID | 파일명 | 비고 |
|-------------|----------------|--------|------|
| 사용자 인증 | FE-AUTH-001 | components/auth/auth-page.tsx | 로그인/회원가입 페이지 컴포넌트 |

### 1.3 Frontend - 워크플로우 블록 컴포넌트 (components/blocks/)
...
### 1.7 Frontend - 마이페이지 컴포넌트 (components/mypage/)

| 서브시스템명 | 프로그램파일 ID | 파일명 | 비고 |
|-------------|----------------|--------|------|
| 사용자 관리 | FE-MYP-001 | components/mypage/my-page.tsx | 마이페이지 (프로필, 구매내역, 판매내역) |

### 1.8 Frontend - UI 기본 컴포넌트 (components/ui/)
...
### 1.9 Frontend - 루트 컴포넌트 (components/)
...
### 1.13 Backend - API 라우터 (backend/app/api/)
...
### 1.14 Backend - 데이터 모델 (backend/app/models/)
...
### 1.15 Backend - Pydantic 스키마 (backend/app/schemas/)
...
### 1.16 Backend - 비즈니스 로직 서비스 (backend/app/services/)
...
### 1.17 Backend - 지식베이스 서비스 (backend/app/services/knowledge_base/)
...
### 1.18 Backend - 유틸리티 (backend/app/utils/)
...
### 1.19 Backend - 애플리케이션 코어 (backend/app/)
...
프로그램 코드의 프로그램 목록을 서브시스템별로 분류하여 기술하였다.

## 항목 설명

### 프로그램 목록
- **서브시스템명**: 본 프로그램파일이 관련되는 서브시스템명을 기입
- **프로그램파일 ID**: 프로그램 파일의 고유 ID (FE-: Frontend, BE-: Backend)
- **프로그램파일명**: 프로그램 파일의 상대 경로 및 파일명
- **비고**: 해당 파일의 주요 기능 및 역할 설명
